...
11. **New page using `<main className="space-y-5 p-6">` as wrapper.** The layout shell already provides `<main>` with padding. New pages must return `<div className="space-y-6">` as the root element, not a second `<main>`. Double-wrapping causes content to appear shrunken and misaligned.
12. **Title card missing on new pages.** Every page must have a gradient title card at the top matching the page theme color. Pattern: `<div className="bg-gradient-to-br from-X to-Y rounded-2xl shadow-lg p-6 text-white">` with icon, title (uppercase), and subtitle. See `src/app/finance/page.tsx` or `src/app/surat-jalan/page.tsx` as reference.
13. **Dark mode missing on new pages.** Every element must have a `dark:` variant. Common patterns: `bg-white dark:bg-slate-900`, `text-slate-800 dark:text-slate-100`, `border-slate-200 dark:border-slate-700`, `text-slate-500 dark:text-slate-400`. Never use hardcoded light colors without a dark counterpart.
14. **Icon color same as card gradient background.** Icons inside stat cards must use `text-white`, not the same color as the gradient (e.g. `text-yellow-500` on a yellow card makes the icon invisible). Background patch uses `bg-white/20 rounded-lg`, icon uses `text-white`.
15. **Badge/number with `bg-*-50 text-white`.** The `-50` shade is near-white; white text on it is unreadable. Use solid color (`bg-emerald-500`, `bg-blue-500`) for badges that contain white text.
16. **Typo `dark:bg-slate-8000`.** This is not a valid Tailwind class (extra zero). The correct class is `dark:bg-slate-800`. Always verify dark mode class names are valid Tailwind tokens.
