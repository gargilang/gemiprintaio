  ...
  await db.transaction(async () => {
    ...
    if (refundAmount > 0) {
      await insertCashbookEntry({
        ...
        kredit: refundAmount,
        keperluan: `Refund retur penjualan ${nomor} (${sale.nomor_invoice}) [REF:${returnId}]`,
    ...
    if (nonCashReversal > 0) {
      ...
      await insertCashbookEntry({
        ...
        keperluan: `Pembalik omzet retur ${nomor} (${sale.nomor_invoice}) [REF:${returnId}]`,
        catatan: input.catatan || input.reason,
        dibuat_oleh: input.actor_id || null,
    ...
    if (totalHpp > 0) {
      await insertCashbookEntry({
        ...
        debit: totalHpp,
        keperluan: `Pembalik HPP retur ${nomor} (${sale.nomor_invoice}) [REF:${returnId}]`,
        catatan: input.catatan || input.reason,
        dibuat_oleh: input.actor_id || null,
