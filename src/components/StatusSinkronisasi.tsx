...
export default function StatusSinkronisasi({
  className = "",
}: SyncStatusProps) {
  ...
  useEffect(() => {
    ...
    const checkPending = async () => {
      try {
        const status = getClientSyncStatus(lastSync);
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      ...
      <button
        ...
        className="px-3 py-1 text-sm bg-amber-50 dark:bg-slate-8000 text-white rounded hover:bg-amber-600 disabled:opacity-50 disabled:cursor-not-allowed"
      >
