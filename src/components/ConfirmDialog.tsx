...
export default function ConfirmDialog({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-[70] flex items-center justify-center p-4 animate-in fade-in duration-200">
      ...
      >
        <div
          className={`p-6 border-b border-gray-200 dark:border-slate-800 bg-gradient-to-r ${styles.gradient} rounded-t-2xl flex items-center justify-between gap-3 shrink-0`}
        >
        ...
        <div className="p-6">
          <p className="text-base text-gray-700 dark:text-slate-300 whitespace-pre-line leading-relaxed">
            {message}
          </p>
        </div>

        <div className="p-6 border-t border-gray-200 dark:border-slate-800 flex items-center justify-end gap-3 flex-wrap">
          {cancelText && (
            <button
              type="button"
              onClick={onCancel}
              className="px-6 py-3 border-2 border-gray-300 dark:border-slate-700 text-gray-700 dark:text-slate-200 rounded-xl hover:bg-gray-100 dark:hover:bg-slate-800 transition-all font-semibold"
            >
              {cancelText}
            </button>
          )}
