...
export default function PurchaseOrdersPage() {
  ...
  async function submit() {
    ...
    try {
      ...
      setItems([]);
      setCatatan("");
      setNotice("PO tersimpan.");
      await reload();
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Gagal menyimpan PO");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function confirmReceive() {
    ...
    if (!window.confirm(
      `Posting penerimaan PO ${receiveModal.po.nomor_po}?\nIni akan membuat pembelian dan menambah stok.`
    )) {
      return;
    }
    setSaving(true);
    try {
      ...
      setNotice("Penerimaan PO masuk ke pembelian.");
      await reload();
    } catch (error) {
...
<div class="meta">
  ...
  <div style="text-align:right;">
    ...
    <div>Expected: ${po.expected_date || "-"}</div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-indigo-500 to-violet-600 rounded-2xl shadow-lg p-6 text-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold uppercase tracking-wide">Purchase Order</h2>
              <p className="text-white/90 text-sm">Draft, kirim, cetak, dan penerimaan parsial.</p>
            </div>
          </div>
      ...
      <section className="grid gap-4 xl:grid-cols-[420px_1fr]">
        <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-sm">
          <h2 className="mb-3 text-base font-semibold text-slate-800 dark:text-slate-100">Buat Purchase Order</h2>
          <div className="space-y-3">
            ...
            <button disabled={saving} className="w-full rounded-md bg-indigo-600 px-4 py-2 font-medium text-white disabled:opacity-60 hover:bg-indigo-700 transition-colors" onClick={submit}>Simpan Purchase Order</button>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {loading ? (
            ...
            <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 text-sm text-slate-500 dark:text-slate-400 shadow-sm">Belum ada Purchase Order.</div>
              ...
              <div key={po.id} className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-sm">
                <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button className="rounded border border-slate-300 dark:border-slate-600 px-2 py-1 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors" onClick={() => printPo(po)}>Print</button>
                    <button disabled={saving || po.status === "CANCELLED" || po.status === "RECEIVED"} className="rounded border border-slate-300 dark:border-slate-600 px-2 py-1 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-50 transition-colors" onClick={() => updatePurchaseOrderStatusAction(po.id, "SENT").then(() => reload())}>Mark Sent</button>
                    <button disabled={saving || po.status === "CANCELLED" || allReceived} className="rounded bg-emerald-600 px-2 py-1 text-sm text-white disabled:opacity-50 hover:bg-emerald-700 transition-colors" onClick={() => openReceive(po)}>Receive</button>
                  </div>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 dark:bg-slate-800 text-left text-slate-600 dark:text-slate-300">
                      <tr><th className="p-2">Barang</th><th className="p-2 text-right">PO</th><th className="p-2 text-right">Diterima</th><th className="p-2 text-right">Sisa</th></tr>
                    </thead>
                    <tbody>
                      {(po.items || []).map((item: any) => (
                        <tr key={item.id} className="border-t border-slate-100 dark:border-slate-800 text-slate-800 dark:text-slate-200">
                          <td className="p-2">
                            {item.barang_id ? (
                              <Link className="text-cyan-600 dark:text-cyan-400 underline-offset-2 hover:underline" href={`/materials?id=${item.barang_id}`}>{item.barang_nama || item.barang_id}</Link>
                            ) : (item.barang_nama || "-")}
                          </td>
        ...
        <div className="fixed inset-0 z-30 flex items-center justify-center bg-slate-900/50 p-4">
          <div className="w-full max-w-2xl rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 shadow-xl">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-100">Penerimaan {receiveModal.po.nomor_po}</h2>
            ...
            <div className="mt-4 max-h-72 overflow-auto rounded-md border border-slate-200 dark:border-slate-700">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 dark:bg-slate-800 text-left text-slate-600 dark:text-slate-300">
                  <tr><th className="p-2">Barang</th><th className="p-2 text-right">PO</th><th className="p-2 text-right">Sudah</th><th className="p-2 text-right">Terima</th></tr>
                </thead>
                <tbody>
