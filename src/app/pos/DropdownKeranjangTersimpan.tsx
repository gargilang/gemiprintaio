...
export default function DropdownKeranjangTersimpan({
  ...
  const popup = (
    ...
    >
      {showWarning && (
        <p className="text-xs text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/20 px-2 py-1.5 rounded mb-2">
          Finalisasi atau jadikan penawaran dulu — sudah lebih dari 30 keranjang
          tersimpan.
        </p>
      )}
      {sorted.length === 0 ? (
        <p className="text-sm text-slate-500 dark:text-slate-400 px-2 py-3 text-center">
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <span
                    className={`inline-block mt-1 text-[11px] px-1.5 py-0.5 rounded font-bold uppercase ${
                      status === "KEDALUWARSA"
