...
export default function SalesReturnsPage() {
  const {
    ...
    isLoading,
    mutate,
  } = useCachedData<any>("sales-returns-init", getSalesReturnInitAction);
  ...
  async function submit() {
    ...
    try {
      ...
      setNotice(
        error instanceof Error ? error.message : "Gagal retur penjualan",
      );
    } finally {
      setSaving(false);
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/90 text-base">
                Stok kembali, piutang/refund dihitung otomatis, HPP dibalik.
              </p>
            </div>
          </div>
          {notice ? (
          ...
          <div className="space-y-3">
            ...
            >
              ...
              {data.sales.map((saleRow: any) => (
                <option key={saleRow.id} value={saleRow.id}>
                  {saleRow.nomor_faktur} - {money(saleRow.total_jumlah)}
                </option>
              ))}
            </select>
            {sale ? (
            ...
            <button
              disabled={saving}
              className="w-full rounded-md bg-rose-600 px-4 py-2.5 font-medium text-white disabled:opacity-60 hover:bg-rose-700 transition-colors"
              onClick={submit}
            >
              Posting Retur
            </button>
          ...
          <table className="w-full text-base">
            <thead className="bg-slate-50 dark:bg-slate-800 text-left text-slate-600 dark:text-slate-300">
              <tr>
                ...
                <th className="p-3 text-right">Piutang ↓</th>
              ...
              {loading ? (
                ...
                data.returns.map((retur: any) => (
                  ...
                  >
                    ...
                    <td className="p-3">
                      {retur.source?.id ? (
                        <Link
                          ...
                          href={`/pos?invoice=${retur.source.id}`}
                        >
                    ...
                    <td className="p-3 text-right">
                      {money(retur.receivable_reduction)}
                    </td>
                    <td className="p-3 text-right">
