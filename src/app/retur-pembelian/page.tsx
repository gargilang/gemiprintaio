...
import { createPurchaseReturnAction, getPurchaseReturnInitAction } from "./actions";
...
export default function PurchaseReturnsPage() {
  ...
  async function submit() {
    const lines = Object.entries(qtyByItem)
      .map(([item_pembelian_id, qty]) => ({ item_pembelian_id, qty: Number(qty || 0) }))
    ...
    setSaving(true);
    try {
      await createPurchaseReturnAction({ purchase_id: purchaseId, reason, items: lines });
      setQtyByItem({});
      setReason("");
      ...
      <div className="bg-gradient-to-br from-amber-500 to-orange-600 rounded-2xl shadow-lg p-6 text-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold uppercase tracking-wide">Retur Pembelian</h2>
              <p className="text-white/90 text-sm">Stok keluar, hutang dikurangi dulu, refund vendor dicatat ke kas.</p>
            </div>
          </div>
      ...
      <section className="grid gap-4 lg:grid-cols-[460px_1fr]">
        <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-sm">
          ...
          <div className="space-y-3">
            <select className="w-full rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 p-2" value={purchaseId} onChange={(e) => { setPurchaseId(e.target.value); setQtyByItem({}); }} disabled={saving}>
              <option value="">Pilih pembelian</option>
              {data.purchases.map((row: any) => (
                <option key={row.id} value={row.id}>{row.nomor_pembelian || row.nomor_faktur} - {money(row.total_harga || row.total_jumlah)}</option>
              ))}
            </select>
            {purchase ? (
              <div className="space-y-2">
                {(purchase.items || []).map((item: any) => (
                  <div key={item.id} className="grid grid-cols-[1fr_80px_90px] gap-2 rounded-md bg-slate-50 dark:bg-slate-800 p-2 text-sm">
                    <span className="truncate text-slate-800 dark:text-slate-100">{item.nama_barang || item.barang_id}</span>
              ...
              <div className="rounded-md border border-dashed border-slate-300 dark:border-slate-600 p-3 text-center text-xs text-slate-500 dark:text-slate-400">Pilih pembelian untuk lihat item.</div>
            )}
            ...
            <button disabled={saving} className="w-full rounded-md bg-amber-600 px-4 py-2 font-medium text-white disabled:opacity-60 hover:bg-amber-700 transition-colors" onClick={submit}>Posting Retur</button>
          </div>
        </div>
        ...
        <div className="overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-slate-800 text-left text-slate-600 dark:text-slate-300">
              <tr><th className="p-3">Nomor</th><th className="p-3">Pembelian</th><th className="p-3 text-right">Retur</th><th className="p-3 text-right">Hutang ↓</th><th className="p-3 text-right">Refund</th></tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td className="p-4 text-slate-500 dark:text-slate-400" colSpan={5}>Memuat...</td></tr>
              ) : data.returns.length === 0 ? (
                <tr><td className="p-4 text-slate-500 dark:text-slate-400" colSpan={5}>Belum ada retur pembelian.</td></tr>
                ...
                <tr key={retur.id} className="border-t border-slate-100 dark:border-slate-800 hover:bg-slate-50/60 dark:hover:bg-slate-800/60 text-slate-800 dark:text-slate-200">
                  ...
                  <td className="p-3">
                    {retur.source?.id ? (
                      <Link className="text-cyan-600 dark:text-cyan-400 underline-offset-2 hover:underline" href={`/pembelian?id=${retur.source.id}`}>
                        {retur.source.nomor_pembelian || retur.source.nomor_faktur || retur.pembelian_id}
                      </Link>
                    ) : retur.pembelian_id}
                  </td>
                  <td className="p-3 text-right">{money(retur.total_retur)}</td>
                  <td className="p-3 text-right">{money(retur.debt_reduction)}</td>
