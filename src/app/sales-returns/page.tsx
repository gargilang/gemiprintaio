...
export default function SalesReturnsPage() {
  ...
  async function submit() {
    ...
    if (!saleId || lines.length === 0 || !reason.trim()) {
      return setNotice("Pilih invoice, qty, dan alasan retur.");
    }
    if (!window.confirm(
      `Posting retur penjualan?\nIni akan: stok kembali, piutang dikurangi, refund kas (kalau invoice sudah terbayar).`
    )) return;
    setSaving(true);
    try {
          ...
          <div className="space-y-3">
            <select className="w-full rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 p-2" value={saleId} onChange={(e) => { setSaleId(e.target.value); setQtyByItem({}); }} disabled={saving}>
              ...
              {data.sales.map((saleRow: any) => (
                <option key={saleRow.id} value={saleRow.id}>{saleRow.nomor_invoice} - {money(saleRow.total_jumlah)}</option>
              ))}
            </select>
            {sale ? (
              ...
              <div className="rounded-md border border-dashed border-slate-300 dark:border-slate-600 p-3 text-center text-xs text-slate-500 dark:text-slate-400">Pilih invoice untuk lihat item.</div>
            )}
        ...
        <div className="overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-slate-800 text-left text-slate-600 dark:text-slate-300">
              <tr><th className="p-3">Nomor</th><th className="p-3">Invoice</th><th className="p-3 text-right">Retur</th><th className="p-3 text-right">Piutang ↓</th><th className="p-3 text-right">Refund</th></tr>
            </thead>
            <tbody>
              {loading ? (
                  ...
                  <td className="p-3">
                    {retur.source?.id ? (
                      <Link className="text-cyan-600 dark:text-cyan-400 underline-offset-2 hover:underline" href={`/pos?invoice=${retur.source.id}`}>
                        {retur.source.nomor_invoice || retur.penjualan_id}
                      </Link>
                    ) : retur.penjualan_id}
                  </td>
