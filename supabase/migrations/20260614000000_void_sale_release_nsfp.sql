-- ════════════════════════════════════════════════════════════════════════════
-- Migration: void_sale_with_inventory — lepas NSFP saat void
--
-- BUG: saat penjualan ber-PPN di-void, nomor seri faktur pajak (nsfp_pool)
-- yang dikunci createSale (TERSEDIA -> TERPAKAI) TIDAK dilepas. Faktur pajak
-- batal, tapi nomor serinya "hangus" TERPAKAI dan tidak bisa dipakai lagi.
--
-- FIX: setelah penjualan ditandai VOIDED, lepas semua NSFP yang menunjuk
-- penjualan ini (TERPAKAI -> TERSEDIA, penjualan_id -> NULL), konsisten dengan
-- compensateFailedSale di pos-mutations.ts.
--
-- Catatan: badan fungsi disalin PERSIS dari migrasi 20260613000000 (immutable),
-- hanya menambahkan SATU blok UPDATE nsfp_pool sebelum RETURN. Migrasi lama
-- tidak boleh diedit; perbaikan jalur web HARUS lewat file migrasi baru ini.
-- ════════════════════════════════════════════════════════════════════════════
