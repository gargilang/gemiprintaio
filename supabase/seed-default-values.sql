...
-- Default admin (username: gemi, password: admin) — SHA-256 hex, matches app /api/auth/login
...
INSERT INTO kategori_barang (id, nama, butuh_spesifikasi_status, urutan_tampilan) VALUES
...
INSERT INTO subkategori_barang (id, kategori_id, nama, urutan_tampilan) VALUES
...
VALUES
  ('fin-cat-gaji', 'GAJI', 'Gaji', 'bg-teal-100', 'text-teal-800', 'border-teal-300', 'kredit', 1, 130, '[{"column":"biaya_operasional","amount_field":"kredit","sign":1}]'::jsonb),
  ('fin-cat-pinjaman-karyawan', 'PINJAMAN_KARYAWAN', 'Pinjaman Karyawan', 'bg-cyan-100', 'text-cyan-800', 'border-cyan-300', 'both', 1, 140, '[]'::jsonb)
ON CONFLICT (category_code) DO NOTHING;
...
INSERT INTO spesifikasi_cepat_barang (id, kategori_id, tipe_spesifikasi, nilai_spesifikasi, urutan_tampilan) VALUES
...
INSERT INTO spesifikasi_cepat_barang (id, kategori_id, tipe_spesifikasi, nilai_spesifikasi, urutan_tampilan)
SELECT
...
-- pos-mutations.ts (barang_id 'barang-jasa-maklon'); tanpa baris ini semua
-- penjualan maklon gagal FK di web. Disamakan dengan seed SQLite.
INSERT INTO barang
