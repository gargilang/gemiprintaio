...
class _MaterialFormDialogState extends ConsumerState<MaterialFormDialog> {
    ...
    if (widget.existing != null) {
      ...
      for (final p in m.harga) {
        _prices.add(_PriceRow(
          ...
          hargaBeliCtrl:
              TextEditingController(text: p.hargaBeli.toStringAsFixed(0)),
          hargaMemberCtrl:
              TextEditingController(text: p.hargaMember.toStringAsFixed(0)),
  ...
  void _addPriceRow() {
    setState(() => _prices.add(_PriceRow(
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final body = {
        ...
        'dimensi_required': _dimensiRequired,
        ...
        'harga': _prices
            .map((r) => {
                  ...
                  'harga_jual':
                      double.tryParse(r.hargaJualCtrl.text) ?? 0,
                  'harga_beli':
                      double.tryParse(r.hargaBeliCtrl.text) ?? 0,
                  'harga_member':
                      double.tryParse(r.hargaMemberCtrl.text) ?? 0,
                  'faktor_konversi':
                      double.tryParse(r.faktorCtrl.text) ?? 1,
                ...
                children: [
                  ...
                  if (_filteredSubcategories.isNotEmpty) ...[
                    ...
                    DropdownButtonFormField<String?>(
                      ...
                      decoration:
                          const InputDecoration(labelText: 'Sub-Kategori'),
                      items: [
                        const DropdownMenuItem(
                            value: null, child: Text('-- Tanpa Sub-Kategori --')),
                        ..._filteredSubcategories.map((s) => DropdownMenuItem(
                  ...
                  SwitchListTile(
                    ...
                    title: const Text('Butuh Dimensi (Panjang × Lebar)',
                        style: TextStyle(fontSize: 14)),
                    subtitle: const Text(
                        'Aktifkan untuk banner, spanduk, dll.',
                        style: TextStyle(fontSize: 12)),
                    value: _dimensiRequired,
                    onChanged: (v) => setState(() => _dimensiRequired = v),
                  ),
                  SwitchListTile(
                    dense: true,
                    contentPadding: EdgeInsets.zero,
                        ...
                        child: Column(
                          children: [
                            ...
                            TextFormField(
                              ...
                              decoration:
                                  const InputDecoration(labelText: 'Nama Satuan *', isDense: true),
                            ),
                            const SizedBox(height: 8),
                            Row(
                              children: [
