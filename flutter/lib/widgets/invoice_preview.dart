...
class InvoicePreview {
  static Future<void> show(BuildContext context, {
    ...
    doc.addPage(pw.MultiPage(pageFormat: PdfPageFormat.roll80, build: (ctx) => [
      pw.Header(level: 0, child: pw.Text(title, style: pw.TextStyle(font: bold, fontSize: 14))),
      if (invoiceNumber != null) pw.Text('No: $invoiceNumber', style: pw.TextStyle(font: font, fontSize: 10)),
      if (date != null) pw.Text('Tanggal: $date', style: pw.TextStyle(font: font, fontSize: 10)),
      if (customerName != null) pw.Text('Pelanggan: $customerName', style: pw.TextStyle(font: font, fontSize: 10)),
      if (customerAddress != null) pw.Text(customerAddress, style: pw.TextStyle(font: font, fontSize: 9)),
      pw.SizedBox(height: 10),
      ...lines.map((line) => pw.Row(children: [
      ...
      if (additionalCharges.isNotEmpty) ...[
        ...
        ...additionalCharges.map((c) => pw.Row(children: [
...
class InvoiceLine {
  ...
  const InvoiceLine({required this.name, required this.qty, required this.price, required this.subtotal});
